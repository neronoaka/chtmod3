...
public class WTHmr extends WTitem {
	public WTHmr() {
		this.setUnlocalizedName("WTHmr");
		this.setRegistryName("WTHmr");
	}
					...
					for (int x = newPos1.getX(); x <= newPos2.getX(); x++) {
						for (int y = newPos1.getY(); y <= newPos2.getY(); y++) {
							for (int z = newPos1.getZ(); z <= newPos2.getZ(); z++) {
								worldIn.setBlockState(new BlockPos(x, y, z), worldIn.getBlockState(pos));
							}
						}
					}
					playerIn.addChatMessage(
							new TextComponentString(area + "Blocks" + I18n.format("gui.changed", new Object[0])));
				} else {
					playerIn.addChatMessage(
